% Adaptive trapezoidal formula using error estimation by mesh halving
% INPUT
...
function [int, flag, stats] = trapAdpt(f, a, b, tol, hMin)
    % Allow extra stat computations to be disabled since these are really slow
    getStats = false;
    if nargout == 3
        getStats = true;
    ...
    function int = approxInterval(left, right, leftVal, rightVal, localTol, level)
        ...
        % Compute the midpoint of the interval and the corresponding value of f
        middle = (right + left) / 2;
        middleVal = f(middle);

        % Compute (error) estimates
        coarseEst = h * (leftVal + rightVal) / 2;
        fineEst = h / 4 * (leftVal + 2 * middleVal + rightVal);
        errEst = abs(fineEst - coarseEst) / 3;
